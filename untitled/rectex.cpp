...
void RectEx::setCtrlRects()
{
    ...
    */
    ...
    if (status != RS_ROTATING)//非旋转情况
    {
        ...
        int h = (l + r) / 2;
        int v = (t + b) / 2;
        if(!ctrlRects.isEmpty())ctrlRects.clear();
    ...
    {   if(ctrlRects.isEmpty()){
        ...
        for (i = 0; i<4; i++)//位于边中点的控制框
        {
            ...
            ctrlRects.replace(i*2+2, QRect(QPoint((pt1.x()+pt2.x())/2-5, (pt1.y()+pt2.y())/2-5),
                                           QPoint((pt1.x()+pt2.x())/2+5, (pt1.y()+pt2.y())/2+5)));
        }
...
void RectEx::rotateRect(QPoint p1, QPoint p2, bool change)
{
    ...
    //生成新坐标
    for (int i = 0; i<4; i++)
        pointstemp.replace(i, QPoint((int)(R[0][0] * points[i].x() + R[0][1] * points[i].y() + R[0][2]),
